...
- (id)initARTKPwithWidth:(int) width andHeight:(int)height andNumTrackedPatterns: (int) numTrackedPatterns
{
	if (width < 0 || height < 0)
	{
	...
	//  - can detect a maximum of "numTrackedPatterns" patterns in one image, higher number increase accuracy, decreases performance
	tracker = new TrackerMultiMarker(width, height, numTrackedPatterns, 6, 6, 6, 0);
...
- (void)initTrackerWithCameraFile:(NSString*)camera_path andMarkerFile:(NSString*)marker_path
{
	...
	if (!tracker->init(camera_cstr, marker_cstr, 1.0f, 1000.0f)) 
	{
...
{
	if (tracker->calc(cameraBuffer) > 0)
	{
 ...
 */
- (TrackerMultiMarker*)getTracker
{
	return tracker;
}
- (void)setTracker:(TrackerMultiMarker*)newTracker
{
	delete tracker;
	tracker = newTracker;
}
