...
- (id) initWithLayer:(CAEAGLLayer *)layer depth:(BOOL) depth {
	if (self = [super init]) {
		...
		// Set up the initial scene offset to be equal to the identity matrix.
		for (int i = 0; i < 16; i++)
		{
			marker_offset[i] = 0;
		}
...
{	
	...
	glMatrixMode(GL_MODELVIEW);
	glLoadMatrixf(ogl_camera_matrix);		// model view matrix also from ARTPK, updated in separate delegate method
	glMultMatrixf(marker_offset);			// transform model view matrix by Scene Offset matrix sent from server
	// ---------------------------------------------------
...
/**
 * Draw loop Part 3. Part 2 is "draw()" method in EAGLView.
 */
- (void) finishRender
{	
	glPopMatrix();
...
/**
 * Delegate method called from AsyncSocketClientWrapper, updates the 
 * offset matrix with the new offset sent from server.
 */
- (void) updateOffset:(float[]) offset
{
	for (int i = 0; i < 16; i++)
	{
		marker_offset[i] = offset[i];
	}
}
...
- (void) processFrameAt:(unsigned char *) baseAddress withWidth:(size_t) width andHeight:(size_t) height
{
	...
	if(artwrapper == nil) {
		...
		NSString* marker_path = [[NSBundle mainBundle] pathForResource:@"markerboard_480-499" ofType:@"cfg"];
		
		LindsayARAppDelegate* delegate = (LindsayARAppDelegate*)[[UIApplication sharedApplication] delegate];
		
		artwrapper = [[ARToolKitPlusWrapper alloc] initARTKPwithWidth:width andHeight:height andNumTrackedPatterns: delegate.numTrackedMarkers];
		if (artwrapper != nil)
		{
			[artwrapper initTrackerWithCameraFile:camera_path andMarkerFile:marker_path];
	...
	[artwrapper detectMarker:ogl_camera_matrix];
}
