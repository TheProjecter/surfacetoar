...
#ifndef __ARTOOLKITPLUS_TRACKERMULTIMULTIMARKERIMPL_HEADERFILE__
#define __ARTOOLKITPLUS_TRACKERMULTIMULTIMARKERIMPL_HEADERFILE__
 ...
 */
class TrackerMultiMultiMarker: public Tracker {
public:
    /**
     ...
     */
    TrackerMultiMultiMarker(int imWidth, int imHeight, int maxImagePatterns = 8, int pattWidth = 6, int pattHeight = 6, int pattSamples = 6,
            int maxLoadPatterns = 0);

    virtual ~TrackerMultiMultiMarker();
     ...
     */
    virtual int calc(const unsigned char* nImage, int index);
     ...
     *  returns NULL.
     */
    virtual const ARMultiMarkerInfoT* getMultiMarkerConfig(int index) const {
        return markerInfo[index];
    }
	
	
	bool addMarker(const char* nMultiFile);

protected:
    int numDetected;
    bool useDetectLite;

    vector <ARMultiMarkerInfoT*> markerInfo;
